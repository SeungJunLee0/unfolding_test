...
def get_matrix_from_TH2(h2):
    n = h2.GetNbinsX()
    m = h2.GetNbinsY()
    mat = np.zeros((n, m))
    for i in range(1, n+1):
        for j in range(1, m+1):
            mat[i-1][j-1] = h2.GetBinContent(i, j)
    return mat

def bayesian_unfold(response, meas, n_iter=4):
    nbins = meas.GetNbinsX()
    A = get_matrix_from_TH2(response)

    # Normalize columns of A to make conditional P(reco | true)
    A = A / A.sum(axis=0, where=A.sum(axis=0)!=0, keepdims=True)

    M = np.array([meas.GetBinContent(i+1) for i in range(nbins)])

    # Initial prior: flat or measured
    T = np.ones(nbins)
    T = T / T.sum()

    for it in range(n_iter):
        # denominator per reco bin
        denom = A @ T
        denom[denom == 0] = 1e-12  # avoid divide by zero

        # unfolding step
        T_new = np.zeros(nbins)
        for j in range(nbins):
            for i in range(nbins):
                T_new[j] += T[j] * A[i][j] * M[i] / denom[i]
        T = T_new
        T = T / T.sum()  # normalize

    # Create output TH1
    hUnfolded = meas.Clone("hUnfolded_Bayes")
    for i in range(nbins):
        hUnfolded.SetBinContent(i+1, T[i] * meas.Integral())  # rescale to match event count
...
def main():
    ...
    n_iter = 4
    hUnfolded = bayesian_unfold(hResponse2D, hMeas, n_iter)
    ...
    # ---------- 결과 ROOT 파일로 저장 ----------
    outFile = ROOT.TFile("unfold_top_mass_bayesian.root", "RECREATE")
    hTrue.Write()
    hMeas.Write()
    hResponse2D.Write()
    hUnfolded.Write()
    ...
    c3.Write()
    outFile.Close()
    print("모든 객체가 unfold_top_mass_bayesian.root 에 저장되었습니다.")
